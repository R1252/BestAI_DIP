...
//  DIP
//
//  Created by Ray Septian Togi on 2024/7/30.
//
...
class CapturedImageSampler {
    ...
    init(frame: ARFrame) throws {
        ...
        rgbSize = BufferDimension(width: ySize.width, 
                                  height: ySize.height,
                                  bytesPerRow: ySize.width * 4)
    }
    ...
    /// - Returns: A vector_float4 with RGBA values, based on whether or not valid coordinates were supplied.
    ///
    ...
    func getColor(atX x: CGFloat, y: CGFloat) -> vector_float4 {
        ...
        let ty = Int(y * CGFloat(rgbSize.height))
//        let tx = Int(xFlipped * CGFloat(rgbSize.width))
        let tx = Int(x * CGFloat(rgbSize.width))
        
        // Calculate the starting index.
        let index = ty * rgbSize.bytesPerRow + tx * 4
        
        // Grab the ARGB bytes and convert them into scalar CGFloats.
        let a = Float(rgb[index]) / 255.0
        let r = Float(rgb[index + 1]) / 255.0
        let g = Float(rgb[index + 2]) / 255.0
        let b = Float(rgb[index + 3]) / 255.0
